...
validation1 <- filter(validation, d == 1)
validation0 <- filter(validation, d == 0)
...
alphas <- glm(x~d, family = "binomial", data = validation)$coef

case_control_model <- inla_is_conditional(formula_moi = d ~ w,
                              formula_imp = w ~ d,
                              alpha = alphas,
                              MC_1 = M1, MC_0 = M0,
